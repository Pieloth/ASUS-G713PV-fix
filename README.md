...
|Random reboots|1/ Due to misconfiguration between Chipset drivers and AsMedia Firmware. </br>IMPERATIVE to rerun AsMedia Hotfix 2006_1E EACH TIME after modifying Chipsets drivers, to align AsMedia Firmware with new Chipset drivers.</br>2/ Changing Core Isolation / Memory Integrity state (On or Off) seems to mess the Mediatek Bluetooth driver. Bluetooth LE devices sometimes get laggy, and may trigger then Random reboots. The Workaround is simply to reinstall the Mediatek Bluetooth driver if such situation is seen.|
